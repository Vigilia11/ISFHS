    ...
    @foreach($user as $facilitator)
        <div class="row justify-content-center">
                ...
                <div class="row">
                    <div class="col-md-7">
                        <div class="w-100">
                            <div class="fullname mb-4" style="">
                                ...
                                <span class="email">{{ $facilitator->email }}</span>
                            </div>
                            <div class="personal-info">
                                {{ $facilitator->birthdate }} <br>
                                {{ $facilitator->sex }} <br>
                                {{ $facilitator->mobile_number }} <br>
                                {{ $facilitator->barangay }}, {{ $facilitator->city }}, {{ $facilitator->province }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-5 d-flex justify-content-center align-items-center">
                        <div class="profile-picture">
                            <img src="{{ asset('images/user/'.$facilitator->profile_picture) }}" class="w-100 h-100" alt="">
                        </div>
                    </div>
                </div>
                <div class="mb-1 text-secondary account-status-display">
                    Account Status: {{ $facilitator->account_status }}
                </div>
                <div class="w-100 action-button">
                    <input type="text" name="hidden_user_id" value="{{ $facilitator->user_id }}" id="hidden_user_id">
                    <input type="text" name="hidden_account_id" id="hidden_account_id" value="{{ $facilitator->account_id }}">
            ...
            <div class="col-md-9 bg-white shadow mb-5 p-3 " style="boder:1px solid #e5e7eb">
                <div class="row g-3">
                    <img src="{{ asset('images/id/'.$facilitator->front_id) }}" alt="" class="col-md-6" style="height:400px">
                    <img src="{{ asset('images/id/'.$facilitator->back_id) }}" alt="" class="col-md-6" style="height:400px">
                </div>
            </div>
        </div>
        @endforeach    
