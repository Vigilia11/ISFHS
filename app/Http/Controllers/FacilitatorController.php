...
class FacilitatorController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->hasRole('admin')){
            ...
            $facilitators = DB::table('users')
                            ...
                            ->select('users.id', 'profiles.first_name', 'profiles.last_name', 'profiles.picture', 'accounts.status','roles.name as role')
                            ->orderBy('users.created_at', 'DESC')
                            ->get();

            return view('admin.facilitators', compact('facilitators'));
        }
    }
    ...
    public function facilitator($id){
        if(Auth::user()->hasRole('admin')){
            ...
            return view('admin.facilitator', compact('user'));
        }
    }
    public function fetchFacilitator($id){
        if(Auth::user()->hasRole('admin')){
            $facilitator =DB::table('users')
                        ->join('profiles', 'users.id', '=', 'profiles.user_id')
                        ->join('accounts', 'users.id', '=', 'accounts.user_id')
                        ->where('users.id', $id)
                        ->select(
                            'users.id as user_id', 'users.email as email',
                            'profiles.first_name', 'profiles.last_name', 'profiles.sex', 'profiles.birthdate', 'profiles.mobile_number','profiles.barangay', 'profiles.city','profiles.province','profiles.picture as profile_picture',
                            'accounts.front_id','accounts.back_id','accounts.status as account_status'
                            )
                        ->get();
    ...
    public function fetchFacilitators(){
        if(Auth::user()->hasRole('admin')){
